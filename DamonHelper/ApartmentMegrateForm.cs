...
namespace DamonHelper
{
    public partial class ApartmentMegrateForm : Form
    {
        ...
        private async void btn_Megrate_Click(object sender, EventArgs e)
        {
            ...
            {
                await MegrateAsync(apartmentId, tenancyId);
            }
...
where ApartmentId = '{apartmentId}'";
                    ...
                    Dictionary<string, string> tempDic = new Dictionary<string, string>
                    {
                        ...
                        { "s21", s21 }, { "s22", s22 }, /*{"s23",s23 },*/ { "s24", s24 }, { "s25", s25 }, { "s26", s26 }, { "s27", s27 }, { "s28", s28 }, { "s29", s29 }
                    };
                    ...
                    try
                    {
                        foreach (KeyValuePair<string, string> kv in tempDic)
                        {
                            ...
                            int rows = await connection.ExecuteAsync(kv.Value, null, tran);
                            //if (rows <= 0)
                            //{
                            //    tran.Rollback();
                            //    return;
