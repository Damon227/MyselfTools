...
namespace CQ.Foundation.DynamicLinq
{
        ...
        public static IQueryable OrderBy(this IQueryable source, string ordering, params object[] values)
        {
            ...
            foreach (DynamicOrdering o in orderings)
            {
                queryExpr = Expression.Call(
                    typeof(Queryable), o._ascending ? methodAsc : methodDesc,
                    new[] { source.ElementType, o._selector.Type },
                    queryExpr, Expression.Quote(Expression.Lambda(o._selector, parameters)));
                methodAsc = "ThenBy";
    ...
    public static class DynamicExpression
    {
        ...
        {
            return ClassFactory.s_instance.GetDynamicClass(properties);
        }
    ...
    internal class Signature : IEquatable<Signature>
    {
        ...
        public bool Equals(Signature other)
        {
            ...
            {
                if (_properties[i].Name != other._properties[i].Name ||
                    _properties[i].Type != other._properties[i].Type)
                {
                    return false;
                }
            }
    ...
    {
        public static readonly ClassFactory s_instance = new ClassFactory();
    ...
    {
        private static readonly Type[] s_predefinedTypes =
        {
            typeof(object),
            typeof(bool),
            typeof(char),
        ...
        private static readonly Expression s_trueLiteral = Expression.Constant(true);
        private static readonly Expression s_falseLiteral = Expression.Constant(false);
        private static readonly Expression s_nullLiteral = Expression.Constant(null);

        private static readonly string s_keywordIt = "it";
        private static readonly string s_keywordIif = "iif";
        private static readonly string s_keywordNew = "new";

        private static Dictionary<string, object> s_keywords;
...
#pragma warning disable 0219
        public IEnumerable<DynamicOrdering> ParseOrdering()
        {
            ...
            while (true)
            {
                ...
                orderings.Add(new DynamicOrdering { _selector = expr, _ascending = ascending });
                if (_token._id != TokenId.Comma)
                {
                    break;
                }
                NextToken();
...
#pragma warning restore 0219
        ...
        private Expression ParseLogicalOr()
        {
            ...
            while (_token._id == TokenId.DoubleBar || TokenIdentifierIs("or"))
            {
                ...
                CheckAndPromoteOperands(typeof(ILogicalSignatures), op._text, ref left, ref right, op._pos);
                   ...
                   _token._id == TokenId.LessThan || _token._id == TokenId.LessThanEqual)
            {
                ...
                if (isEquality && !left.Type.IsValueType && !right.Type.IsValueType)
                {
                    if (left.Type != right.Type)
                    {
                        ...
                        {
                            throw IncompatibleOperandsError(op._text, left, right, op._pos);
                        }
                    }
                }
                ...
                else
                {
                    CheckAndPromoteOperands(isEquality ? typeof(IEqualitySignatures) : typeof(IRelationalSignatures),
                        op._text, ref left, ref right, op._pos);
                }
                switch (op._id)
                {
                    case TokenId.Equal:
                   ...
                   _token._id == TokenId.Amphersand)
            {
                ...
                switch (op._id)
                {
                    case TokenId.Plus:
                        ...
                        CheckAndPromoteOperands(typeof(IAddSignatures), op._text, ref left, ref right, op._pos);
                        left = GenerateAdd(left, right);
                        break;
                    case TokenId.Minus:
                        CheckAndPromoteOperands(typeof(ISubtractSignatures), op._text, ref left, ref right, op._pos);
        ...
        private Expression ParseMultiplicative()
        {
                   ...
                   _token._id == TokenId.Percent || TokenIdentifierIs("mod"))
            {
                ...
                CheckAndPromoteOperands(typeof(IArithmeticSignatures), op._text, ref left, ref right, op._pos);
                switch (op._id)
                {
        ...
        private Expression ParseUnary()
        {
                ...
                TokenIdentifierIs("not"))
            {
                ...
                {
                    CheckAndPromoteOperand(typeof(INegationSignatures), op._text, ref expr, op._pos);
                    expr = Expression.Negate(expr);
                }
                else
                {
                    CheckAndPromoteOperand(typeof(INotSignatures), op._text, ref expr, op._pos);
        ...
        private Expression ParseStringLiteral()
        {
            ...
            char quote = _token._text[0];
            string s = _token._text.Substring(1, _token._text.Length - 2);
            int start = 0;
            while (true)
            {
        ...
        private Expression ParseIdentifier()
        {
            ...
            object value;
            if (s_keywords.TryGetValue(_token._text, out value))
            {
                if (value is Type)
                {
                    return ParseTypeAccess((Type)value);
                }
                if (value == s_keywordIt)
                {
                    return ParseIt();
                }
                if (value == s_keywordIif)
                {
                    return ParseIif();
                }
                if (value == s_keywordNew)
                {
                    return ParseNew();
                }
                NextToken();
            ...
            if (_symbols.TryGetValue(_token._text, out value) ||
                _externals != null && _externals.TryGetValue(_token._text, out value))
            {
            ...
            throw ParseError(Res.UnknownIdentifier, _token._text);
        }
        ...
        private Expression GenerateConditional(Expression test, Expression expr1, Expression expr2, int errorPos)
        {
            ...
            {
                Expression expr1As2 = expr2 != s_nullLiteral ? PromoteExpression(expr1, expr2.Type, true) : null;
                Expression expr2As1 = expr1 != s_nullLiteral ? PromoteExpression(expr2, expr1.Type, true) : null;
                ...
                {
                    string type1 = expr1 != s_nullLiteral ? expr1.Type.Name : "null";
                    string type2 = expr2 != s_nullLiteral ? expr2.Type.Name : "null";
                    if (expr1As2 != null)
                    {
        ...
        {
            MethodData[] applicable = methods.Select(m => new MethodData { _methodBase = m, _parameters = m.GetParameters() }).Where(m => IsApplicable(m, args)).ToArray();
            if (applicable.Length > 1)
            {
                MethodData[] applicable1 = applicable;
                applicable = applicable.Where(m => applicable1.All(n => m == n || IsBetterThan(args, m, n))).ToArray();
            }
            if (applicable.Length == 1)
            {
        ...
        {
            if (method._parameters.Length != args.Length)
            {
                return false;
            }
        ...
        private void NextToken()
        {
            ...
            _token._id = t;
            _token._text = _text.Substring(tokenPos, _textPos - tokenPos);
            _token._pos = tokenPos;
        }

        private bool TokenIdentifierIs(string id)
        {
            return _token._id == TokenId.Identifier && string.Equals(id, _token._text, StringComparison.OrdinalIgnoreCase);
        }

        private string GetIdentifier()
        {
            ValidateToken(TokenId.Identifier, Res.IdentifierExpected);
            string id = _token._text;
            if (id.Length > 1 && id[0] == '@')
            {
                id = id.Substring(1);
            }
        ...
        private static Dictionary<string, object> CreateKeywords()
        {
            ...
            {
                { "true", s_trueLiteral },
                { "false", s_falseLiteral },
                { "null", s_nullLiteral },
                { s_keywordIt, s_keywordIt },
                { s_keywordIif, s_keywordIif },
                { s_keywordNew, s_keywordNew }
            };
        ...
        private interface IRelationalSignatures : IArithmeticSignatures
        {
            ...
            void F(DateTimeOffset x, string y);
            void F(TimeSpan x, TimeSpan y);
            void F(char? x, char? y);
            void F(DateTime? x, DateTime? y);
            void F(DateTimeOffset? x, DateTimeOffset? y);
